...
<script lang="ts">
  /* imports */
  import { type PossibleThemeType, themes } from '../../../types/common'
  import { changeTheme, getTheme } from '../../../support/cookies'
  ...
  /* constants */
  let currentTheme = getTheme().text
  /* callbacks */
  function colorChange(value: PossibleThemeType, text: string) {
    dropdownOpen = false
    changeTheme(value)
    currentTheme = text
  }
...
<div class="container" bind:this={containerRef}>
  ...
  <div class="dropdown-container">
    <button class="dropdown" on:click={dropdownClick}>
      <span class="dropdown-selected">{currentTheme}</span>
    ...
    >
      {#each themes as { text, value }}
      <button
        on:click|preventDefault={() => colorChange(value, text)}
